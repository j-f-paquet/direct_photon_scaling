...
pTmin, slope, intercept = np.transpose(np.loadtxt(os.path.join("./calcs/photons/ipg_kompost_cent0060_20pctbins/beta_vs_pT_cut_AuAu200_source=prompt.dat")))
plt.plot(pTmin, slope, ":", color="grey", label="Prompt")

pTmin, slope, intercept = np.transpose(np.loadtxt(os.path.join("./calcs/photons/ipg_kompost_cent0060_varied_cents/beta_vs_pT_cut_AuAu200_source=prompt.dat")))
mask=pTmin>3.5
plt.plot(pTmin[mask], slope[mask], ":", color="grey", label="")


pTmin, slope, intercept = np.transpose(np.loadtxt(os.path.join("./calcs/photons/ipg_kompost_cent0060_20pctbins/beta_vs_pT_cut_AuAu200_source=prompt_plus_kompost_plus_hydro_thermal_with_visc_T105_reg1_suppr_tauChem1.dat")))
plt.plot(pTmin, slope, "-", color="Red", label="Prompt+thermal")

pTmin, slope, intercept = np.transpose(np.loadtxt(os.path.join("./calcs/photons/ipg_kompost_cent0060_varied_cents/beta_vs_pT_cut_AuAu200_source=prompt_plus_kompost_plus_hydro_thermal_with_visc_T105_reg1_suppr_tauChem1.dat")))
mask=pTmin>3.5
