...
filename="./calcs/photons/ipg_kompost_cent0060_20pctbins/dNh_vs_dNg_AuAu200_phenix_Ncoll_source=prompt_plus_kompost_plus_hydro_thermal_with_visc_T105_reg1_suppr_tauChem1_pTcut=1.0.dat"
dNch, dNg = np.transpose(np.loadtxt(filename))
plt.plot(dNch, dNg,"-", color='black', label="Prompt+pre.eq+thermal [slope=1.32]")
...
filename="./calcs/photons/ipg_kompost_cent0060_20pctbins/dNh_vs_dNg_AuAu200_phenix_Ncoll_source=prompt_pTcut=1.0.dat"
dNch, dNg = np.transpose(np.loadtxt(filename))
plt.plot(dNch, dNg,"--", color='blue', label="Prompt [slope=1.14]")
